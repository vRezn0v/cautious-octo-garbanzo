...
exports.comment = function (req, res) {
    Post.update({
        ...
        if (result.length) {
            console.log(`[${req.params.hub}] number of comment updated!`)
        }
    })
    ...
    Comment({
        ...
        console.log(`[${req.params.hub}] comment posted!`)
...
exports.vote = function (req, res) {
    if (req.body.action == "increment") {
        ...
        Profile.update({
            ...
            $inc: {
                comment_chadpoints: 1
            }
        }, function (err, result) {
            if (err) throw err;

            if (result) {
                console.log(`[${req.session.user}] comment chadpoints increased!`)
            }
        });
        ...
        Profile.update({
            ...
            if (result) {
                console.log(`[${req.session.user}] comment chadpoints decreased!`)
            }
        });
    }
